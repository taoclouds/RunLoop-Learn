...
//有 source0 source1
// source0 代表的是 APP 内部事件，如 UIEvent、CFSocket 都是 source0类型的
// 当一个 source0事件准备执行的时候，它必须是 signal 状态
...
/*
* RunLoop 与线程一一对应，对应的方式是以 key-Value 形式存储在一个全局字典__CFRunLoops中。
* 主线程的 RunLoop 会在初始化全局字典的时候创建
* 子线程的 RunLoop 会在第一次获取的时候创建，如果没获取的话就会一直不创建
* RunLoop 会在线程结束时自动销毁
**/
...
// 向 commonmode 里面添加 item，即向 mode 添加事件
...
/*
* 如果是 commonmode，则该 source 会保存到 commonmodeitem 中
*
****/
...
//向 RunLoop 添加观察者，用于观察 RunLoop 的状态
